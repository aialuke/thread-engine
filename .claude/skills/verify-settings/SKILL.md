...
3. **Write `drafts/<slug>/CLAIMS.md`:**
   ...
   **The operator's own run** (build-log and tool-verdict claims such as "the pipeline made this script in 4 minutes" or "Grok caught 3 bugs Codex missed", and each tool-swap row's test such as "opened a layered PSD in Photopea"): Source URL is `operator, <date>` plus the artifact that shows it, a screenshot in the draft's `images/` or a log file path. Vendor-stated or tested: `tested`. No artifact means the row is `VERIFY`.
