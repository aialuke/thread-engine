...
5. Otherwise write `reviews/week-YYYY-Www.md` (ISO week, Australia/Brisbane) with these sections, each a few lines:
   ...
   - **Lanes to review.** Posts the daily run recorded on its own (slug `x-<id>`, lane `other`) and any post whose lane looks wrong: test each against the three tests in `reference/audience.md` and propose `main` or `other` in one line. While `main` covers tech broadly, a PAID → FREE post with every row tested and sourced, and a quote of a maker's release, can be `main`. Post 1 of PAID → FREE (`2102737637346095128`) qualifies once its Resolve row is tested (`shipped/paid-to-free-tools/CLAIMS.md`). On the operator's yes, run `python3 scripts/loop.py set-lane --root-id <id> --lane <lane> --reason "<which test>"`.
   ...
   - **Profile.** Run `python3 scripts/x_api.py me`. Say whether the bio and the pinned post match `reference/audience.md`. The pin should be the newest PAID → FREE post that's live, tested, sourced and unboosted (operator, 24 Sep 2026); until one exists, nothing boosted. If not, suggest a one-sentence bio and a pin with no link or a well-known link (algorithm fact A14).
